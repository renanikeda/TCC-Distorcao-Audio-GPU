...
int main(int argc, char *argv[]){
    ...
    file_out = sf_open ("tempo_atualizado.wav", SFM_WRITE, &sfinfo_out); //Determina o arquivo de saída
    ...
    const int N_amostras = 48000; //Para verficacao da timeline
    ...
    // }
    sf_read_double (file_in, amostra_in_host, N_amostras);
    ...
    printf("nro repeticoes de amostra = %d\n", nro_repeticoes_de_amostra);
    
    //Arquivo de log
    FILE *fp = fopen("time_gpu_L=30_5k.csv","w");
    FILE *fp_amostras = fopen("time_gpu_L=30_5k_amostras.csv","w");
    fprintf(fp,"N_estagios,N_operacoes_case01,N_operacoes_case2,Média Tempo [s]\n");
    fprintf(fp_amostras,"N_estagios,N_operacoes_case01,N_operacoes_case2,Tempo [s]\n");
                ...
                for(int i = 0; i < nro_repeticoes_de_amostra; i++)
                {
                    ...
                    while (amostras_processadas_totais< N_amostras)
                    {
                        if(amostras_processadas[0]< N_buffer)
                        {
                            ...
                            if(amostras_enviadas < N_amostras)
                            {
                    ...
                    time_out = clock();
                    time = ((double)(time_out - time_in)) / (CLOCKS_PER_SEC);
                    
                    
                    fprintf(fp_amostras,"%d,%d,%d,%f\n", N_estagios, N_operacoes_case0, N_operacoes_case2, time);
                    // printf("N_estagios: %d N_operacoes_case0: %d N_operacoes_case2: %d Tempo [s]: %f\n", N_estagios, N_operacoes_case0, N_operacoes_case2, time);
                    time_media+=time;

                }
                contador++;

                time_media=time_media/(nro_repeticoes_de_amostra);
            
                fprintf(fp,"%d,%d,%d,%f\n", N_estagios, N_operacoes_case0, N_operacoes_case2, time_media);
                printf("N_estagios: %d N_operacoes_case0: %d N_operacoes_case2: %d Tempo [s]: %f\n", N_estagios, N_operacoes_case0, N_operacoes_case2, time_media);
