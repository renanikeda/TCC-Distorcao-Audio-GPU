...
int main(int argc, char *argv[])
{
    ...
    double delay = 0;
    double delay_max = 0;
    double soma_total_media=0;
    int num_iteracoes=0;
    int read_count=1;

    FILE *fp = fopen("delay_cpu_L=30_5k", "w");
    FILE *fp_amostras = fopen("delay_cpu_L=30_5k_amostras", "w");
    fprintf(fp, "N_distorcao, delay_medio");
    fprintf(fp_amostras, "N_distorcao, delay");
    ...
    for(int N_distorcao = 144; N_distorcao<=5040; N_distorcao=N_distorcao+144)
    {
        ...
        for(int z=0; z<nro_repeticoes_de_amostra;z++)
        {
            ...
            file_out = sf_open ("sweep_48khz_delay_cpu.wav", SFM_WRITE, &sfinfo_out); //Determina o arquivo de saída
            sf_command (file_out, SFC_SET_CLIPPING, NULL, SF_TRUE) ;

            while (num_iteracoes<1000)
            {
                inicio_total = clock();       
                ...
                delay = (((double)(fim_total - inicio_total)) / CLOCKS_PER_SEC)*1e3;
                
                soma_total_media+=delay;

                fprintf(fp_amostras, "%d, %f \n",N_distorcao, delay);
        ...
        soma_total_media=soma_total_media/(nro_repeticoes_de_amostra*1000);

        printf("N_distorcoes: %d Delay_media [ms]: %f\n", N_distorcao,soma_total_media);
